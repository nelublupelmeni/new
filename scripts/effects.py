...
def add_sepia(image, strength=0.8):
    ...
    sepia_img = cv2.transform(image, sepia_filter) #применяет линейную матрицу к каждому 3-канальному пикселю изображения
    sepia_img = np.clip(sepia_img, 0, 255).astype(np.uint8) #ограничивает все элементы массива в диапазоне [0, 255], чтобы не было отрицательных или слишком больших значений.
    output = cv2.addWeighted(image, 1 - strength, sepia_img, strength, 0) #вычисляет поэлементное взвешенное суммирование что-то типо альфа-смешивания
    return output
